...
int main(int argc, char* argv[]){
    ...
    if(inputError == 0){
        ...
        if(p>1){    
                    ...
                    for(int subStep = 0; subStep < numSubSteps; subStep++){
                        ...
                        for(int src = 1; src < p; src++){
                            ...
                            for(int i = tempStartIndex1; i < (tempNumParticles1+tempStartIndex1); i++){
                                particles[i] = tempParticles[i];//tempParticles[i-tempStartIndex1];
                            }
            ...
            //all other nodes do this
                ...
                for(int cnt = 0; cnt < glob_steps; cnt++){
                    ...
                    MPI_Recv(&tempParticles     , totalParticles, MPI_VEC3, 0, 1, MPI_COMM_WORLD, &status);
                    ...
                    for(int outerDot = tempStartIndex; outerDot < (tempStartIndex + tempNumParticles); outerDot++){
                        ...
                        for(int innerDot = 0; innerDot < totalParticles; innerDot++){
                            ...
                            if(innerDot != outerDot){
                                magX=pow(tempParticles[outerDot].getX() - tempParticles[innerDot].getX(),2);
                                magY=pow(tempParticles[outerDot].getY() - tempParticles[innerDot].getY(),2);
                                magZ=pow(tempParticles[outerDot].getZ() - tempParticles[innerDot].getZ(),2);
                                
                                currentForceX = tempParticles[innerDot].getMass()*(tempParticles[outerDot].getX()-tempParticles[innerDot].getX())/(pow(sqrt(magX+magY+magZ),3));
                                currentForceY = tempParticles[innerDot].getMass()*(tempParticles[outerDot].getY()-tempParticles[innerDot].getY())/(pow(sqrt(magX+magY+magZ),3));
                                currentForceZ = tempParticles[innerDot].getMass()*(tempParticles[outerDot].getZ()-tempParticles[innerDot].getZ())/(pow(sqrt(magX+magY+magZ),3));
                            }
                    ...
                    for(int index = tempStartIndex; index < (tempNumParticles+tempStartIndex); index++){
                        ...
                        currentMass = tempParticles[index].getMass();
                        totForceX =  0;
                        totForceY =  0;
                        totForceZ =  0;
                        forcesX = 0;
                        ...
                        posX = tempParticles[index].getX() + glob_timeSubStep * tempParticles[index].getVelocityX();
                        posY = tempParticles[index].getY() + glob_timeSubStep * tempParticles[index].getVelocityY();
                        posZ = tempParticles[index].getZ() + glob_timeSubStep * tempParticles[index].getVelocityZ();

                        velX = tempParticles[index].getVelocityX() + ((glob_timeSubStep*totForceX)/ tempParticles[index].getMass());
                        velY = tempParticles[index].getVelocityY() + ((glob_timeSubStep*totForceY)/ tempParticles[index].getMass());
                        velZ = tempParticles[index].getVelocityZ() + ((glob_timeSubStep*totForceZ)/ tempParticles[index].getMass());
                        
                        tempVec = vec3(posX,posY,posZ,tempParticles[index].getR(),tempParticles[index].getG(),tempParticles[index].getB(),tempParticles[index].getMass(),velX, velY, velZ);
                        tempParticles[index] = tempVec;
                    ...
                    MPI_Send(&tempParticles     , totalParticles, MPI_VEC3, 0, 2, MPI_COMM_WORLD);
        ...
        //////////////////////////////////////////////////////////////
        //This next step is only done if the implementation needs to be serial because only 1 processor was given
        if(p == 1){
